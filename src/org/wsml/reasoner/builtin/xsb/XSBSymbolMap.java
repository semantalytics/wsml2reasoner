...
public class XSBSymbolMap {
    ...
    public String uri2String(String URI){
        if (uri2name.get(URI)!=null)
            ...
            int i = URI.length();
            while (Character.isLetter(URI.charAt(i-1)))i--;
...
//            System.out.println("#"+name+"\t\t"+URI);
    ...
    public String term2String(Term t){
        if (t instanceof ConstructedTerm){
            ...
            //return "V"+v.getName();
            return Character.toUpperCase(v.getName().charAt(0))+
                v.getName().substring(1);
        }else{
            return null;
        }
    }
    ...
    public Term convertToWSML(TermModel xsbTerm){
        ...
        if (!xsbTerm.isLeaf()){
            ...
            return wsmoManager.getDataFactory().createWsmlInteger(xsbTerm.toString());
