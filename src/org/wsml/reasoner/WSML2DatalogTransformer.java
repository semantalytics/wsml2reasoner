...
public class WSML2DatalogTransformer {
	...
	private static class DatalogVisitor extends
		...
		protected boolean bodyImplication;
		...
		public void handleAtom(Atom atom) {
			...
			// TODO implies / impliedBy !!!
//			if (bodyImplication) {
//				Literal l = (atom.getArity() > 0) ? new Literal(false, predUri,
//						atom.listParameters()) : new Literal(false, predUri,
//						new ArrayList<Term>());
//				storeLiteral(l);
//				inHeadOfRule = false;
//				inBodyOfRule = false;
...
//			}
		...
		private void storeLiteral(Literal l) {
			...
			if (rules.size() == 0) {
				rules.add(new Rule(null, new LinkedList<Literal>()));
			}
			if (inBodyOfRule) {
				if (!fact) {
					rules.set(rules.size() - 1, addToBody(rules.get(rules
							.size() - 1), l));
					for(Rule r: rules){
						if(r.getBody().size() == 0){
							rules.set(rules.indexOf(r), addToBody(rules.get(rules.indexOf(r)), l));
						}
					}
				} else {
					LinkedList<Literal> ll = new LinkedList<Literal>();
					ll.add(l);
					rules.add(new Rule(null, ll));
					fact = false;
				}
			} else if (inHeadOfRule) {
				if (rules.get(rules.size() - 1).getHead() == null) {
					List<Literal> bl = rules.get(rules.size() - 1).getBody();
					rules.remove(rules.size() - 1);
					rules.add(new Rule(l, bl));
				} else {
		...
		private Rule addToBody(Rule r, Literal l) {
			List<Literal> ll = r.getBody();
			if (ll == null || ll.size() == 0) {
				ll = new LinkedList<Literal>();
			}
			ll.add(l);
			return new Rule(r.getHead(), ll);
		}
		...
		public void handleLogicProgrammingRule(LogicProgrammingRule arg0) {
			inHeadOfRule = false;
			inBodyOfRule = true;
			if (arg0.getRightOperand() instanceof Implication) {
				bodyImplication = true;
			}
		}
