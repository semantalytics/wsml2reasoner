...
public class EllyReasonerExample {
	...
	public void doTestRun() throws Exception {
		...
		Ontology exampleOntology = loadOntology("locations.wsml");
		if (exampleOntology == null)
			return;

		// get A reasoner
		DLReasoner reasoner = DefaultWSMLReasonerFactory.getFactory()
				.createDL2Reasoner(null);
		...
		IRI identifier = container.getWsmoFactory().createIRI(
				"http://www.example.org/example/country");
		Set<Instance> instances = reasoner.getInstances(container
				.getWsmoFactory().createConcept(identifier));

		System.out.println("Instances of country:");
		System.out.println(instances);

		identifier = container.getWsmoFactory().createIRI(
				"http://www.example.org/example/city");
		Set<Concept> subConcepts = reasoner.getSubConcepts(container
				.getWsmoFactory().createConcept(identifier));
	...
	void doHumanTestRun() throws Exception {
		...
		Ontology exampleOntology = loadOntology("human.wsml");
		if (exampleOntology == null)
			return;
		...
		identifier = container.getWsmoFactory().createIRI(
				"http://www.example.org/example/hasName");
		System.out.println("Concepts of hasName: "
				+ reasoner.getConceptsOf(identifier));
		// System.out.println("RangesOfConstraintAttribute of hasName: " +
		// reasoner.getRangesOfConstraintAttribute(identifier));
		System.out.println("RangesOfInferingAttribute of hasName: "
				+ reasoner.getRangesOfInferingAttribute(identifier));
