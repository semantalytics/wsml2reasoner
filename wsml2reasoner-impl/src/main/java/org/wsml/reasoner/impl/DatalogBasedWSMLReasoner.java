...
public class DatalogBasedWSMLReasoner implements LPReasoner {
	 ...
	 * @param factory
	 *            the wsmo4j factory to pass to the constructor
	 * @param config
	 ...
	 */
	private DatalogReasonerFacade createFacade(BuiltInReasoner builtInType,
			FactoryContainer factory, Map<String, Object> config)
			throws InternalReasonerException {
	...
	/*
	 * Helper method for handleAttributeInheritance() See the note before that
	 * method.
	 */
	...
	// "An error occurred while propogating attributes to sub-concepts", ex );
	// }
	// }
	// }
	//
	// // Now recurse all the way down to concepts that don't have any
	// sub-concepts
	// for (Concept subConcept : concept.listSubConcepts()) {
	...
	// }
	/*
	 * Add all attributes of superconcepts to the subconcepts This is a BAD way
	 * to do it! This is now accomplished with two new rules in
	 * WSML2DatalogTransformer, but in case these new rules cause some problems
	 * this code will be left for a little while.
	 */
	...
	public void registerOntologies(Set<Ontology> ontologies)
		...
		IRI newIdentifier = wsmoFactory
				.createIRI("urn:org.wsml.reasoner.impl.DatalogBasedWSMLReasoner:registeredOntology");

		// FIXME dw: ontology imports do not work, if they'd work they'd need to
		// be merged as well
	...
	protected Set<org.wsml.reasoner.ConjunctiveQuery> convertQuery(
		...
		for (Rule rule : p) {
			if (rule.getHead().getPredicateUri().equals(WSML_RESULT_PREDICATE)) {
				...
				// a program
				// an materializes the result, since the materialization needs
				// to
				// be done all
