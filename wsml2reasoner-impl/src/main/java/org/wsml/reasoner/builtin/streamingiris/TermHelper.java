...
import at.sti2.streamingiris.terms.concrete.SqName;
...
public class TermHelper {
	 ...
	 */
	public static Term convertTermFromIrisToWsmo4j(final ITerm t,
		...
		if (container == null) {
			throw new IllegalArgumentException(
					"The factory container must not be null");
		}

		WsmoFactory wsmoFactory = container.getWsmoFactory();
		LogicalExpressionFactory leFactory = container
		...
		if (t instanceof IIri) {
			...
			return leFactory.createConstructedTerm(
					wsmoFactory.createIRI(ct.getFunctionSymbol()), terms);
		} else {
	 ...
	 *             if the term-type couldn't be converted
	 */
	public static DataValue convertDataValueFromIrisToWsmo4j(final ITerm t,
			FactoryContainer container) {
		if (t == null) {
			throw new IllegalArgumentException("The term must not be null");
		}
		...
		if (t instanceof IAnyURI) {
			...
			return dataFactory.createBase64Binary(((IBase64Binary) t)
					.getValue().getBytes());
			...
			return dataFactory.createDate(dt.getYear(), dt.getMonth(),
					dt.getDay(), tzData[0], tzData[1], tzData[2]);
		} else if (t instanceof IDateTimeStamp) {
			final IDateTimeStamp dt = (IDateTimeStamp) t;
			int[] tzData = getTZData(dt.getTimeZone());
			return dataFactory.createDateTimeStamp(dt.getYear(), dt.getMonth(),
					dt.getDay(), dt.getHour(), dt.getMinute(),
					dt.getDecimalSecond(), tzData[0], tzData[1], tzData[2]);
		} else if (t instanceof IDateTime) {
			final IDateTime dt = (IDateTime) t;
			int[] tzData = getTZData(dt.getTimeZone());
			return dataFactory.createDateTime(dt.getYear(), dt.getMonth(),
					dt.getDay(), dt.getHour(), dt.getMinute(),
					dt.getDecimalSecond(), tzData[0], tzData[1], tzData[2]);
		} else if (t instanceof IYearMonthDuration) {
			final IYearMonthDuration dt = (IYearMonthDuration) t;

			return dataFactory.createYearMonthDuration(dt.getValue().getSign(),
					dt.getYear(), dt.getMonth());
		} else if (t instanceof IDayTimeDuration) {
			final IDayTimeDuration dt = (IDayTimeDuration) t;

			return dataFactory.createDayTimeDuration(dt.getValue().getSign(),
					dt.getDay(), dt.getHour(), dt.getMinute(),
					dt.getDecimalSecond());
		} else if (t instanceof IDuration) {
			// it is essential that IDuration comes after IDayTimeDuration and
			// IYearMonthDuration
			// since those interfaces are extend IDuration
			final IDuration dt = (IDuration) t;

			return dataFactory.createDuration(dt.getValue().getSign(),
			...
			return dataFactory.createGregorianMonthDay(md.getMonth(),
					md.getDay());
			...
			return dataFactory.createGregorianYearMonth(md.getYear(),
					md.getMonth());
		} else if (t instanceof IHexBinary) {
			return dataFactory.createHexBinary(((IHexBinary) t).getValue()
					.getBytes());
			...
			return dataFactory.createNormalizedString(((IStringTerm) t)
					.getValue());
			...
			return dataFactory.createTime(time.getHour(), time.getMinute(),
					time.getDecimalSecond(), tzData[0], tzData[1], tzData[2]);

			// Numeric
		} else if (t instanceof IFloatTerm) {
			return dataFactory.createFloat(((IFloatTerm) t).getValue()
					.floatValue());
		} else if (t instanceof IDoubleTerm) {
			return dataFactory.createDouble(((IDoubleTerm) t).getValue()
					.doubleValue());
			...
			return dataFactory
					.createNonPositiveInteger(t.getValue().toString());
			...
			return dataFactory
					.createNonNegativeInteger(t.getValue().toString());
			...
			return dataFactory.createXMLLiteral(((IXMLLiteral) t).getString(),
					lang);
			...
			return dataFactory.createPlainLiteral(
					((IPlainLiteral) t).getString(), lang);
		}
	 ...
	 */
	public static IConcreteTerm convertWsmo4jDataValueToIrisTerm(
			final DataValue v) {
		if (v == null) {
		...
		if (t.equals(WsmlDataType.WSML_BASE64BINARY)
			...
			if (length > 3) {
				...
				return CONCRETE.createDate(getIntFromValue(cv, 0),
						getIntFromValue(cv, 1), getIntFromValue(cv, 2), hours,
						mins);
			} else {
				return CONCRETE.createDate(getIntFromValue(cv, 0),
						getIntFromValue(cv, 1), getIntFromValue(cv, 2), 0, 0);
			}
			...
			if (length > 6) {
				...
				return CONCRETE.createDateTime(getIntFromValue(cv, 0),
						...
						getDoubleFromValue(cv, 5), hours, mins);
			} else {
				return CONCRETE.createDateTime(getIntFromValue(cv, 0),
						getIntFromValue(cv, 1), getIntFromValue(cv, 2),
						getIntFromValue(cv, 3), getIntFromValue(cv, 4),
						getDoubleFromValue(cv, 5), 0, 0);
			}
			...
			if (length > 3) {
				...
				return CONCRETE.createTime(getIntFromValue(cv, 0),
						getIntFromValue(cv, 1), getDoubleFromValue(cv, 2),
						hours, mins);
			} else {
				return CONCRETE
						.createTime(getIntFromValue(cv, 0),
								getIntFromValue(cv, 1),
								getDoubleFromValue(cv, 2), 0, 0);
			}
			...
			checkDuration(getIntFromValue(cv, 0), getIntFromValue(cv, 1),
					getIntFromValue(cv, 2), getIntFromValue(cv, 3),
					getIntFromValue(cv, 4), getIntFromValue(cv, 5),
					getDoubleFromValue(cv, 6));

			return CONCRETE.createDuration(getIntFromValue(cv, 0) >= 0,
					getIntFromValue(cv, 1), getIntFromValue(cv, 2),
					getIntFromValue(cv, 3), getIntFromValue(cv, 4),
					getIntFromValue(cv, 5), getDoubleFromValue(cv, 6));
			...
			checkDuration(getIntFromValue(cv, 0), getIntFromValue(cv, 1),
					getIntFromValue(cv, 2), getIntFromValue(cv, 3),
					getDoubleFromValue(cv, 4));

			return CONCRETE.createDayTimeDuration(getIntFromValue(cv, 0) >= 0,
					getIntFromValue(cv, 1), getIntFromValue(cv, 2),
					getIntFromValue(cv, 3), getDoubleFromValue(cv, 4));
			...
			return CONCRETE.createYearMonthDuration(
					getIntFromValue(cv, 0) >= 0, getIntFromValue(cv, 1),
					getIntFromValue(cv, 2));
			...
			final ComplexDataValue cv = (ComplexDataValue) v;
			return CONCRETE.createQName(getStringFromValue(cv, 0),
					getStringFromValue(cv, 1));
		} else if (t.equals(XmlSchemaDataType.XSD_NOTATION)) {
			final ComplexDataValue cv = (ComplexDataValue) v;
			return CONCRETE.createNOTATION(getStringFromValue(cv, 0),
					getStringFromValue(cv, 1));
	 ...
	 * 
	 * @param sign
	 *            The sign of the duration or time zone.
	 * @param vals
	 *            The values of the duration or time zone.
	 */
