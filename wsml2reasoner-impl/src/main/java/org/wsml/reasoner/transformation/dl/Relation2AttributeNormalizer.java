...
public class Relation2AttributeNormalizer implements OntologyNormalizer {
	...
	public Relation2AttributeNormalizer(FactoryContainer factory) {
		...
		anonymousIdTranslator = new AnonymousIdTranslator(
	 ...
	 */
	private Set<Entity> normalizeRelation(Relation relation)
		...
		for (Type t : p1.listTypes()) {
			if (t instanceof Concept) {
				...
				Attribute attribute = newConcept.createAttribute(relation
						.getIdentifier());
		...
		if (relation.listSuperRelations().size() > 0) {
			Axiom axiom = normalizeSuperRelations(
					relation.listSuperRelations(), relation);
			if (axiom != null) {
				result.add(axiom);
			}
		}
	 ...
	 */
	private Axiom normalizeSuperRelations(Set<Relation> superRelations,
			Relation relation) {
		Axiom result = wsmoFactory
				.createAxiom((Identifier) anonymousIdTranslator
						.translate(wsmoFactory.createAnonymousID()));
		for (Relation sr : superRelations) {
			String le = "?x[_\"" + relation.getIdentifier()
					+ "\" hasValue ?y] implies " + "?x[_\""
					+ sr.getIdentifier() + "\" hasValue ?y].";
	...
	private Set<Entity> normalizeRelationInstance(
			RelationInstance relationInstance) throws InvalidModelException {
		Set<Entity> result = new HashSet<Entity>();
		...
		if (newInstance == null) {
			newInstance = wsmoFactory.createInstance(((Instance) v1)
					.getIdentifier());
		}
		if (v2 instanceof Instance) {
			Instance tmp = wsmoFactory.createInstance(((Instance) v2)
					.getIdentifier());
			newInstance.addAttributeValue(relationInstance.getRelation()
					.getIdentifier(), tmp);
		} else {
			newInstance.addAttributeValue(relationInstance.getRelation()
					.getIdentifier(), v2);
