...
public class OnePassReplacementNormalizer implements
	...
	public OnePassReplacementNormalizer(List<NormalizationRule> preorderRules,
			List<NormalizationRule> postorderRules, FactoryContainer factory) {
	...
	public LogicalExpression normalize(LogicalExpression expression) {
		...
		if (expression instanceof CompoundExpression) {
			...
			for (LogicalExpression argument : compound.listOperands()) {
	...
	protected CompoundExpression replaceArguments(CompoundExpression compound,
		...
		if (compound instanceof Constraint) {
			...
			result = leFactory.createExistentialQuantification(
					((ExistentialQuantification) compound).listVariables(),
					arguments.get(0));
		} else if (compound instanceof UniversalQuantification) {
			result = leFactory.createUniversalQuantification(
					((UniversalQuantification) compound).listVariables(),
					arguments.get(0));
		} else if (compound instanceof Conjunction) {
			result = leFactory.createConjunction(arguments.get(0),
					arguments.get(1));
		} else if (compound instanceof Disjunction) {
			result = leFactory.createDisjunction(arguments.get(0),
					arguments.get(1));
		} else if (compound instanceof Equivalence) {
			result = leFactory.createEquivalence(arguments.get(0),
					arguments.get(1));
		} else if (compound instanceof Implication) {
			result = leFactory.createImplication(arguments.get(0),
					arguments.get(1));
