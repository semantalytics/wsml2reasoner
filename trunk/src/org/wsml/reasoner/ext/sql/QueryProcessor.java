...
/**
 * Accept and manipulate a 'SQL-like' query expression conforming to WSML-Flight-A syntax.
 * 
 * 		SELECT X1 , . . . , Xn
 * 		FROM ontology
 * 		WHERE wsmlQuery
 *
 * where wsmlQuery is a WSML-Flight query, X1 - Xn are a subset of the variables appearing in
 * wsmlQuery and ontology is an ontology confirming to the WSML-Flight variant.
 * 
 * For example:
 * 
 *		SELECT ?actor LIKE baz%, count(?pe),  
 *		FROM "http://www.somewhere.org/somedomainn#someBusinessProcess"
 *		WHERE ?pe [ generatedBy hasValue ? actor ] memberOf evo#successfulExecutionEvent
 *      GROUP BY ?actor
 *      ORDER BY count(?pe)
 *      HAVING count(?pe) > 0
 * 
 * The SQL statement required to retrieve the inermediate results will look like this:
 * 
 *		SELECT C_actor, count(C_pe),  
 *		FROM TABLE_NAME
 *		WHERE C_actor LIKE 'baz%'
 *      GROUP BY C_actor
 *      ORDER BY COUNT(C_pe)
 *      HAVING COUNT(C_pe) > 0
 */
public class QueryProcessor
{
	 ...
	 * @param sqlLikeQuery The SQL like query (containing the WSML query)
	 */
	...
	/**
	 * For the given table name, convert the WSML-Flight-A query in to the full SQL query required
	 * to read the intermediate results from the WSML-Flight query.
	 * @param tableName
	 * @return
	 */
	...
	@throws QueryFormatException If the syntax of the query is invalid.
	*/
	...
	/**
	 * A functor base class for use when concatenating collection of string token.
	 */
	...
	/**
	 * A functor that converts any variable names found in to sql column name equivalents.
	 */
	...
	/**
	 * Convert provided tokens that match a variable name in to a SQL column name, otherwise leave
	 * alone.
	 * @param expression
	 * @return
	 */
	 ...
	 * This is needed in order to get meaningful column names in result sets which include aggregates.
	 * 
	 * E.g. SUM(X) => returns AS SUM_X which can be appended to the original expression:
	 * SUM(X) AS SUM_X. This results in SUM_X as column name in the SQL ResultSet.
	 * 
	...
	/**
	 * Process character events, but filter out single and doubly quoted strings as separate tokens.
	 ...
	 * @param query The query string (WSML-Flight-A)
	 */
	...
	{
		// Simple validation first. Must later traverse the expression list looking for commas
		// in the correct places and 'LIKE' sub expressions.
		// Can also ensure that open and closing braces match around COUNT( expr ), SUM(  ), etc
		if ( sequence.size() < 2 )
	 ...
	 * @throws QueryFormatException If this token can is syntactically incorrect in this position in the query.
	 */
	 ...
	 * @param token true If the supplied token is a SQL keyword.
	 * @return
	 */
	...
	/**
	 * Indicates of the supplied token introduces a new clause in the SQL-like query.
	...
	{
		return
			token.equalsIgnoreCase( SELECT ) ||
			token.equalsIgnoreCase( FROM ) ||
			token.equalsIgnoreCase( WHERE ) ||
			token.equalsIgnoreCase( GROUP ) ||
			token.equalsIgnoreCase( ORDER ) ||
			token.equalsIgnoreCase( HAVING ) ||
			token.equalsIgnoreCase( LIMIT ) ||
			token.equalsIgnoreCase( OFFSET );
	}
	...
	/**
	 * Holder for select expressions, of the form either: ?variable OR <aggregate>( ?variable )
	 */
	...
	private static final String[] ALL_SQL_KEYWORDS =
	{
		SELECT, FROM, WHERE, GROUP, ORDER, HAVING, LIMIT, OFFSET, BY, LIKE,
		COUNT, MIN, MAX, SUM, AVG, AS
	};
