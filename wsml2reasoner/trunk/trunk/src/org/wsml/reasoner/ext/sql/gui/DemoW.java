...
public class DemoW
{
	...
	public static class MainFrame extends JFrame implements ActionListener
	{
		...
		private void setup()
		{
			...
			mQuery
					.setText("SELECT ?child, ?school, ?principle "
							+ NEW_LINE
							+ "FROM _\"http://wsml2reasoner.svn.sourceforge.net/viewvc/*checkout*/wsml2reasoner/wsml2reasoner/trunk/test/files/simpsons.wsml\" "
							+ NEW_LINE
							+ "WHERE ?child[attends hasValue ?school] and ?principle[principleOf hasValue ?school] "
							+ NEW_LINE + "\n" + "\n" + "\n" + "\n");
		...
		synchronized void run()
		{
			...
			mExecutionThread = new Thread(new ExecutionTask(query),
					"query task");
