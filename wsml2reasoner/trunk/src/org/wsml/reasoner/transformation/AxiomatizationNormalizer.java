...
public class AxiomatizationNormalizer implements OntologyNormalizer {
    ...
    public Ontology normalize(Ontology o) {
        ...
        String resultIRI = (o.getIdentifier() != null ? o.getIdentifier()
                .asString()
                + "-as-axioms" : "iri:normalized-ontology-" + o.hashCode());
        ...
        try {
            ...
            for (Object nextNFP : nfps.entrySet()) {
                ...
                result.addNFPValue((org.wsmo.common.IRI) entry.getKey(), entry
                        .getValue());
            }
     ...
     */
    private void handleConcept(Concept c, Ontology o)
        ...
        for (LogicalExpression l : lExprs) {
            Axiom ax = factory.createAxiom(factory.createIRI(axPrefix + "-"
                    + (i++)));
            ax.addDefinition(l);
            o.addAxiom(ax);
        }
    ...
    private Set handleConceptAttribute(Attribute a, Term cTerm)
        ...
        for (Type type : rangeTypes) {
            if (type instanceof Concept) {
                ...
                throw new IllegalArgumentException(
                        "Complex datatype are currently not supported in Normalization to LogicalExpressions");
        ...
        AttrSpecification attrSpec1 = leFactory.createAttrSpecification(
                AttrSpecification.ATTR_VALUE, attID, toSet(y));
        m3 = leFactory.createMolecule(x, null, null, toSet(attrSpec1));

        AttrSpecification attrSpec2 = leFactory.createAttrSpecification(
                AttrSpecification.ATTR_VALUE, attID, toSet(z));
        m4 = leFactory.createMolecule(y, null, null, toSet(attrSpec2));

        m5 = leFactory.createMolecule(x, null, null, toSet(attrSpec2));

        AttrSpecification attrSpec3 = leFactory.createAttrSpecification(
                AttrSpecification.ATTR_VALUE, attID, toSet(x));
        ...
        if (a.getMinCardinality() > 0) {
            ...
            for (int i = 0; i < a.getMinCardinality(); i++) {
                for (int j = i + 1; j < a.getMinCardinality(); j++) {
                    ...
                    body = leFactory.createBinary(CompoundExpression.AND, body,
                            nextInEqStatement);
                }
            }
     ...
     * @throws SynchronisationException
     */
    private void handleConceptInstance(Instance i, Ontology o)
            throws SynchronisationException, InvalidModelException {
        Set<LogicalExpression> lExprs = new HashSet<LogicalExpression>();
    ...
    private org.omwg.logexpression.LogicalExpression handleInstanceAttribute(
        ...
        for (Object obj : aVals) {
            if (obj instanceof Instance) { // THIS SEEMS NOT TO BE USED RIGHT
                // NOW:
    ...
    private org.omwg.logexpression.LogicalExpression handleRelationParameter(
        ...
        if (!rp.isConstraining()) {
            // impliesType
            result = leFactory.createBinary(CompoundExpression.IMPLIES, body,
                    head);
        } else {
            // ofType
    ...
    private IRI convertIRI(Identifier id) {
        if (id instanceof AnonymousID)
            return leFactory.createIRI(AnonymousIdUtils.getNewIri());
        else
            return leFactory.createIRI(id.asString());
    }
    ...
    private Term convertDataValue(DataValue d) {
        ...
        if (d.getType() instanceof SimpleDataType) {
            if (d.getType() instanceof WsmlInteger) {
                java.math.BigInteger bigint = new java.math.BigInteger(d
                        .asString());
            ...
            throw new IllegalArgumentException(
                    "Complex datatype values are at present not supported by the WSMO4j API!");
        }
