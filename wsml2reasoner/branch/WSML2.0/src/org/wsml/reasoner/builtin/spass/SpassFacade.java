...
public class SpassFacade extends FOLAbstractFacade {
    ...
    public SpassFacade(FactoryContainer manager, String endpoint) {
    ...
    public List<EntailmentType> checkEntailment(List<LogicalExpression> conjecture) {
        ...
        for (LogicalExpression le : conjecture) {
            ...
            collector.clear();
            le.accept(collector);
            convertedOntology = convertedOntology.replace("<REPLACE_ME_WITH_MORE_PREDICATES>", getPredicates(map));
            convertedOntology = convertedOntology.replace("<REPLACE_ME_WITH_MORE_FUNCTIONS>", getFunctions(map));
    ...
    private String getPredicates(TPTPSymbolMap symmap) {
        ...
        for (Atom a : collector.getAtoms()) {
            kb += ",(" + symmap.getTPTPTerm(a.getIdentifier().toString()) + "," + a.getArity() + ")";
        }
        return kb;
    }
